...
RSpec.describe Sales::Commands::CreateSale do
  ...
  describe "with command bus" do
    it "publishes event" do
      cmd = described_class.new(sale_id: sale_id, name: "test")
      command_bus.(cmd)
      events = event_store.read.stream(stream).to_a
      expect(events.length).to eq(1)
      expect(events.first).to be_kind_of(SaleCreated)
      expect(events.first.data).to eq(sale_id: sale_id, name: "test")
    end

    it "raises exception when sale already created" do
      cmd = described_class.new(sale_id: sale_id, name: "test")
      command_bus.(cmd)
      expect{ command_bus.(cmd) }.to raise_exception(Sales::Handlers::SaleCreation::AlreadyCreated)
    end
  end
end